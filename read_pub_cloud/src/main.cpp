 ...
 public:
  Reader(std::string file_path)
    ...
    if (access(file_path.c_str(), 0) == -1) {
      ...
      while ((ptr = readdir(dir)) != nullptr) count++;
      closedir(dir);
  ...
  void next_one(pcl::PointCloud<pcl::PointXYZRGBA> &cloud) {
    if (ok) {
      read_file_count++;
      if (read_file_count >= (count - 2) / 4) read_file_count = 0;
      read_one(cloud);
    }
  }
  ...
  void prev_one(pcl::PointCloud<pcl::PointXYZRGBA> &cloud) {
    if (ok) {
      read_file_count--;
      if (read_file_count < 0) read_file_count = (count - 2) / 4 - 1;
      read_one(cloud);
    }
  }
  ...
  void read_one(pcl::PointCloud<pcl::PointXYZRGBA> &cloud) {
    ostringstream oss;
    oss << file_path << std::setfill('0') << "/" << std::setw(4)
        << read_file_count;
...
int main(int argc, char **argv) {
  ...
  ros::Publisher cloud_pub =
      private_nh.advertise<sensor_msgs::PointCloud2>("reader_cloud", 2);
