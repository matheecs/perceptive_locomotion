...
class BalanceController(LeafSystem):
    ...
    def calcTorqueOutput(self, context, output):
        if context.get_time() <= 2.0:
            ...
            # equation (3)
            # size of mass block: 0.4m*0.1m*0.1m
            I_b = np.array(
                [
                    [0.026666666666666672, 0, 0],
                    [0, 0.2266666666666667, 0],
                    [0, 0, 0.2266666666666667],
                ]
            )
            ...
            p_fr_foot = np.array([0.32254366, -0.16987541, 0.0])
            p_hl_foot = np.array([-0.27303829, 0.16202879, 0.0])
            p_hr_foot = np.array([-0.27303829, -0.16202879, 0.0])

            # equation (4)
            mu = 1.0
            prog = MathematicalProgram()
            F = prog.NewContinuousVariables(12, "Forces")
            A = np.zeros((6, 12))
            A[0:3, 0:3] = np.eye(3)
            A[0:3, 3:6] = np.eye(3)
            A[0:3, 6:9] = np.eye(3)
            ...
            A[3:6, 3:6] = skew(p_fr_foot - p_com)
            A[3:6, 6:9] = skew(p_hl_foot - p_com)
            A[3:6, 9:12] = skew(p_hr_foot - p_com)
            alpha = 10.0
            beta = 10.0
            S = 500 * np.eye(6)
            ...
            prog.AddLinearConstraint(F[6] <= mu * F[8])
            prog.AddLinearConstraint(-F[6] <= mu * F[8])
            prog.AddLinearConstraint(F[7] <= mu * F[8])
            prog.AddLinearConstraint(-F[7] <= mu * F[8])

            prog.AddLinearConstraint(F[9] <= mu * F[11])
            prog.AddLinearConstraint(-F[9] <= mu * F[11])
            prog.AddLinearConstraint(F[10] <= mu * F[11])
            prog.AddLinearConstraint(-F[10] <= mu * F[11])
            prog.AddBoundingBoxConstraint(10.0, 4 * mass * g, F[2])
            prog.AddBoundingBoxConstraint(10.0, 4 * mass * g, F[5])
            prog.AddBoundingBoxConstraint(10.0, 4 * mass * g, F[8])
            prog.AddBoundingBoxConstraint(10.0, 4 * mass * g, F[11])
            ...
            F_fr_w = self.F_prev[3:6]
            F_hl_w = self.F_prev[6:9]
            F_hr_w = self.F_prev[9:12]

            joint_q_fl = q_v_estimated[7:10]
            joint_q_fr = q_v_estimated[10:13]
            joint_q_hl = q_v_estimated[13:16]
            joint_q_hr = q_v_estimated[16:19]

            J_fl = fl_jacobian(joint_q_fl)
            J_fr = fr_jacobian(joint_q_fr)
            J_hl = hl_jacobian(joint_q_hl)
            J_hr = hr_jacobian(joint_q_hr)

            tau_fl = -J_fl.T @ R_bw.multiply(F_fl_w)
            tau_fr = -J_fr.T @ R_bw.multiply(F_fr_w)
            tau_hl = -J_hl.T @ R_bw.multiply(F_hl_w)
            ...
            torque_output[3:6] = tau_fr
            torque_output[3:6] = tau_hl
