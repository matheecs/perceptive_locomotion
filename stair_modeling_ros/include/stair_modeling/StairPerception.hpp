...
struct KeyInfo {
  ...
  // if find two cluster vertical plane normal, then
  // second_vertical_plane_normal will be used
...
struct Relation {
  Eigen::Vector3f
      center_direction;  // the center vector diff (no normalization)
...
class StairDetection {
 ...
 private:
  ...
  // the threshold of a1D in paper DIMENSIONALITY BASED SCALE SELECTION IN 3D
  // LIDAR POINT CLOUDS which define the line like of a set of points (0~1)
  float a1D_th;
  ...
  // the vertical_plane_normal should be close to eigen_vector[1] of stair
  // horizontal plane in vertical_plane_normal_est_angle_th
  float vertical_plane_normal_est_angle_th;

  // center vector direction angle with horizontal_plane_direction in 90 degrees
  // threshold the center vector direction should be perpendicular with
  // horizontal_plane_direction
  ...
  // the max distance of two planes in a stair, exceed this distance, the two
  // plane will not be judge as in a stair
  float stair_plane_max_distance;
  // the max angle difference threshold of center vectors of two neighbor planes
  // in stair when modeling
 ...
 public:
  ...
  StairDetection()
      : down_direction(Eigen::Vector3f(0, 0, -1)),
        ...
        ground_height_th(0.06),
        min_ground_plane_points_num(50000),
        merge_dis_th(0.02),
        merge_angle_th(pcl::deg2rad(12.0)),
        merge_distance_th(0.8),
        a1D_th(0.85),
        decay_rate(0.5),
        ...
        plane_min_length(0.4),
        plane_max_length(1.4),
        plane_min_width(0.18),
        plane_max_width(0.80),
        stair_max_height(0.5),
        stair_max_width(0.33),
        stair_plane_max_distance(0.4),
        stair_cv_angle_diff_th(pcl::deg2rad(30.0)) {}
  ...
  void setVertical_plane_normal_est_angle_th(
      float vertical_plane_normal_est_angle_th);
 ...
 private:
  ...
  /*
   * random sample points form full size cloud (construct
   * random_down_sample_cloud in Plane) so that the latter compute can do some
   * approximate computation efficently
   */
  ...
  /*
   * find the dominate vertical plane normal in the current frame (and refine it
   * using history info)
   */
  void findDominateVerticalPlaneNormal(std::vector<Plane> &vector_plane,
                                       KeyInfo &keyInfo);

  /*
   * find a coarse side plane normal using stair steps length direction
   */
  void findHorizontalPlaneDirection(std::vector<Plane> &vector_plane,
                                    KeyInfo &keyInfo);
  ...
  /*
   * find the dominate center diff vector in the current frame (and refine it
   * using history info)
   */
  void findDominateCenterDiffVector(
      const std::vector<Plane> &vector_plane,
   ...
   */
  void filterPlanesWithKeyInfo(std::vector<Plane> &vector_plane,
                               KeyInfo &keyInfo);
   ...
   */
  void findStairPlaneIndexList(std::vector<int2> &fragment_vector,
                               std::vector<int> &stair_plane_index_list);
   ...
   */
  void filterPlanesWithStairPlaneList(std::vector<Plane> &vector_plane,
                                      std::vector<int> &stair_plane_index_list);

  /*
   * compute stair side plane
   */
  void computeStairSidePlane(std::vector<Plane> &vector_plane,
                             KeyInfo &keyInfo);

  /*
   * modeling stair
   */
  bool modelingStair(std::vector<Plane> &vector_plane,
                     std::vector<int> &stair_plane_index_list, KeyInfo &keyInfo,
                     Stair &stair);
   ...
   */
  void computeLineFrom2Planes(const pcl::ModelCoefficients &coefficients_plane1,
                              const pcl::ModelCoefficients &coefficients_plane2,
                              KeyInfo keyInfo, Line &line);

  inline void crossPointOfLineAndPlane(
      const float &x0, const float &y0, const float &z0, const float &a,
      const float &b, const float &c, pcl::ModelCoefficients plane_coefficients,
      pcl::PointXYZ &pc);
};
...
bool StairDetection::process(std::vector<Plane> &vector_plane, Stair &stair,
...
#endif
  // 2. remove vertical planes which maybe vertical poles, and remove slope
  // planes
  ...
  // 4. random sample points form full size cloud (construct
  // random_down_sample_cloud in Plane)
  ...
  t = tic_toc_ms();
  vectime.push_back(t);
  /************\ pre-modeling: get key directions of staircase for
   * modeling********/
  // 9. compute the confuse matrix of each plane (for all the planes in current
  // frame)
  ...
  // 10. find the dominate vertical plane normal in the current frame (add
  ...
  if (!refineKeyDirections(keyInfo)) return has_stair;

#ifdef SHOWTIME
  ROS_INFO("refineKeyDirections time: %.2f ms", tic_toc_ms());
#endif

  // 13. find the dominate center diff vector in the current frame (add dominate
  // center diff vector)
  ...
  has_stair =
      modelingStair(vector_plane, stair_plane_index_list, keyInfo, stair);
  ...
  if (has_stair) computePlaneCounter(stair, keyInfo);
 ...
 */
void StairDetection::removeVerticalPoleSlopePlanes(
    std::vector<Plane> &vector_plane) {
  for (auto &i : vector_plane) {
    if (i.type == Plane::slope) {
      i.ptype = Plane::others;
    } else if (i.type == Plane::vertical) {
      // compute a1D in paper DIMENSIONALITY BASED SCALE SELECTION IN 3D LIDAR
      // POINT CLOUDS
      ...
      if (a1D > a1D_th && a1D > a2D) {
        ...
        Eigen::Vector3f length_vector(
            i.eigen_vectors[2].x, i.eigen_vectors[2].y, i.eigen_vectors[2].z);
        ...
        if ((angle < M_PI / 2 - angle_v) ||
            (M_PI - angle < M_PI / 2 - angle_v)) {
          i.ptype = Plane::others;
          i.info = "{slope}";
        }
      }
...
void StairDetection::sortPlanesByHeight(std::vector<Plane> &vector_plane) {
  ...
  for (size_t i = 0; i < vector_plane.size(); i++) curr_index.push_back(i);
...
/*
 * random sample points form full size cloud (construct random_down_sample_cloud
 * in Plane) so that the latter compute can do some approximate computation
 * efficently
 */
 ...
 * find and merge ground plane
 */
void StairDetection::findMergeGroudPlanes(std::vector<Plane> &vector_plane,
                                          KeyInfo &keyInfo) {
  ...
  if (vector_plane[0].type == Plane::Type::horizontal) {
    ...
    for (auto &point : vector_plane[0].random_down_sample_cloud.points) {
      ...
      if (distance <= ground_height_th) count++;
    }
    ...
    for (int j = i + 1; j < vector_plane.size(); j++) {
      ...
      if (shouldMerge(vector_plane[i], vector_plane[j], merge_dis_th)) {
        if (vector_plane[i].type == vector_plane[j].type) {
          ...
          if (distance > merge_distance_th) continue;
...
bool StairDetection::shouldMerge(Plane &p1, Plane &p2, float dis_th) {
  ...
  if ((angle < merge_angle_th) || (M_PI - angle < merge_angle_th)) {
    ...
    // judge should merge or not
    if (count > (p1.random_down_sample_cloud.points.size() +
                 p2.random_down_sample_cloud.points.size()) /
                        2 -
                    2) {
      return true;
    }
...
void StairDetection::computePlanesMinMax(std::vector<Plane> &vector_plane) {
  for (size_t i = 0; i < vector_plane.size(); i++) {
    vector_plane[i].max[0] = vector_plane[i].max[1] = vector_plane[i].max[2] =
        -FLT_MAX;
    vector_plane[i].min[0] = vector_plane[i].min[1] = vector_plane[i].min[2] =
        FLT_MAX;
    for (size_t index = 0;
         index < vector_plane[i].random_down_sample_cloud.points.size();
         index++) {
      // vector form center to point
      Eigen::Vector3f cp;
      cp[0] = vector_plane[i].random_down_sample_cloud.points[index].x -
              vector_plane[i].center.x;
      cp[1] = vector_plane[i].random_down_sample_cloud.points[index].y -
              vector_plane[i].center.y;
      cp[2] = vector_plane[i].random_down_sample_cloud.points[index].z -
              vector_plane[i].center.z;
      // eigen vectors
 ...
 * frame)
 */
void StairDetection::computeConfuseMatrix(
    const std::vector<Plane> &vector_plane,
    std::vector<std::vector<Relation>> &confuse_matrix) {
  unsigned long d = vector_plane.size();
  confuse_matrix.resize(d);
  for (size_t i = 0; i < d; i++) confuse_matrix[i].resize(d);
...
float StairDetection::minDistaceOfTwoCloud(const Plane &plane1,
                                           const Plane &plane2) {
  ...
  for (auto &i : sample_points_p1) {
    for (auto &j : sample_points_p2) {
      float distance2 = (i.x - j.x) * (i.x - j.x) + (i.y - j.y) * (i.y - j.y) +
                        (i.z - j.z) * (i.z - j.z);
 ...
 * using history info)
 */
void StairDetection::findDominateVerticalPlaneNormal(
    std::vector<Plane> &vector_plane, KeyInfo &keyInfo) {
  ...
  if (!keyInfo.init) {
    if (!keyInfo.main_vertical_plane_normal.normal.norm()) {
      ...
      keyInfo.main_vertical_plane_normal.plane_points_numbers *=
          (1 - decay_rate);
  ...
  for (auto &i : vector_plane) {
    if (i.type == Plane::Type::vertical &&
      ...
      Eigen::Vector3f normal =
          Eigen::Vector3f(i.coefficients.values[0], i.coefficients.values[1],
                          i.coefficients.values[2]);
  ...
  if (!vec_vertical_plane_normals.empty()) {
    ...
    for (auto &vec_vertical_plane_normal : vec_vertical_plane_normals)
      vec_vertical_plane_normal.weight =
          (float)vec_vertical_plane_normal.plane_points_numbers /
          total_point_number;
    ...
    for (auto &vec_vertical_plane_normal : vec_vertical_plane_normals) {
      ...
      for (auto &j : vec_vertical_plane_normals_cluster) {
        ...
        if (angle < vertical_plane_normal_angle_th) {
          ...
          j.count += 1;
          j.plane_points_numbers +=
              vec_vertical_plane_normal.plane_points_numbers;
          exist = true;
          break;
      ...
      if (!exist) {
        ...
        planeNormalCluster.plane_points_numbers =
            vec_vertical_plane_normal.plane_points_numbers;
        vec_vertical_plane_normals_cluster.push_back(planeNormalCluster);
      }
    }

    for (auto &i : vec_vertical_plane_normals_cluster) i.normal.normalize();

    if (!vec_vertical_plane_normals_cluster.empty()) {
      // sort based on the counts
      std::sort(vec_vertical_plane_normals_cluster.begin(),
                vec_vertical_plane_normals_cluster.end(),
      ...
      main_vertical_plane_normal.count =
          vec_vertical_plane_normals_cluster[0].count;
      main_vertical_plane_normal.normal =
          vec_vertical_plane_normals_cluster[0].normal;
      main_vertical_plane_normal.plane_points_numbers =
          vec_vertical_plane_normals_cluster[0].plane_points_numbers;
      ...
      if (vec_vertical_plane_normals_cluster.size() > 1) {
        ...
        second_vertical_plane_normal.count =
            vec_vertical_plane_normals_cluster[1].count;
        second_vertical_plane_normal.normal =
            vec_vertical_plane_normals_cluster[1].normal;
        second_vertical_plane_normal.plane_points_numbers =
            vec_vertical_plane_normals_cluster[1].plane_points_numbers;
 ...
 * find a coarse side plane normal using stair steps length direction
 */
void StairDetection::findHorizontalPlaneDirection(
    std::vector<Plane> &vector_plane, KeyInfo &keyInfo) {
  ...
  if (!keyInfo.init) {
    if (!keyInfo.horizontal_plane_direction.normal.norm()) {
      ...
      vec_horizontal_plane_directions.push_back(
          keyInfo.horizontal_plane_direction);
    }
  }
  ...
  if (!vec_horizontal_plane_directions.empty()) {
    ...
    // for all vertical normals
    for (auto &vec_horizontal_plane_direction :
         vec_horizontal_plane_directions) {
      Eigen::Vector3f &plane_vector_normal =
          vec_horizontal_plane_direction.normal;
      ...
      if (!exist) {
        ...
        hplaneDirCluster.eigen_value =
            vec_horizontal_plane_direction.eigen_value;
        vec_horizontal_plane_direction_cluster.push_back(hplaneDirCluster);
      }
    }

    for (auto &i : vec_horizontal_plane_direction_cluster) i.normal.normalize();

    if (!vec_horizontal_plane_direction_cluster.empty()) {
      // sort based on the counts
      std::sort(vec_horizontal_plane_direction_cluster.begin(),
                vec_horizontal_plane_direction_cluster.end(),
                [&](PlaneNormalCluster a, PlaneNormalCluster b) {
                  return a.count > b.count;
                });
      ...
      horizontal_plane_direction.normal =
          vec_horizontal_plane_direction_cluster[0].normal;
      horizontal_plane_direction.eigen_value =
          vec_horizontal_plane_direction_cluster[0].eigen_value;
...
bool StairDetection::refineKeyDirections(KeyInfo &keyInfo) {
  if (keyInfo.horizontal_plane_direction.normal.norm()) {
    ...
    estimated_vertical_plane_normal =
        keyInfo.horizontal_plane_direction.normal.cross(-1 * down_direction);
    estimated_vertical_plane_normal.normalize();

    // no vertical plane found, then we use horizontal_plane_direction to
    // estimate the vertical_plane_normal
    if (keyInfo.main_vertical_plane_normal.normal.norm() == 0) {
      ...
      float dot_product = keyInfo.main_vertical_plane_normal.normal.dot(
          estimated_vertical_plane_normal);
      float angle = acos(dot_product);

      // if there has a second_vertical_plane_normal
      if (keyInfo.second_vertical_plane_normal.normal.norm()) {
        // compare the two vertical plane normals with
        // horizontal_plane_direction, choose the most match one to refine
        ...
        if (angle2 < angle) {
          angle = angle2;
          keyInfo.main_vertical_plane_normal =
              keyInfo.second_vertical_plane_normal;
        }
      }
 ...
 * using history info)
 */
void StairDetection::findDominateCenterDiffVector(
    const std::vector<Plane> &vector_plane,
    std::vector<std::vector<Relation>> &confuse_matrix, KeyInfo &keyInfo) {
  ...
  if (!keyInfo.init) {
    if (keyInfo.main_center_diff_vector.normal.norm()) {
      ...
      keyInfo.main_center_diff_vector.normal *=
          keyInfo.main_center_diff_vector.weight;
    ...
    if (vector_plane[i].ptype == Plane::pstair_component) {
      for (size_t j = i + 1; j < vector_plane.size(); j++) {
        if (vector_plane[j].ptype == Plane::pstair_component) {
          Eigen::Vector3f normalized_cv_direction =
              confuse_matrix[i][j].center_direction;
  ...
  for (auto &vec_main_center_diff_vector : vec_main_center_diff_vectors) {
    Eigen::Vector3f normalized_cv_direction =
        vec_main_center_diff_vector.normal;
    normalized_cv_direction.normalize();
    ...
    for (auto &vec_cv_direction_cluster : vec_cv_direction_clusters) {
      Eigen::Vector3f normalized_cluster_direction =
          vec_cv_direction_cluster.normal;
      normalized_cluster_direction.normalize();

      float dot_product =
          normalized_cluster_direction.dot(normalized_cv_direction);
      float angle = acos(dot_product);
  ...
  if (!vec_cv_direction_clusters.empty()) {
    // sort based on the counts
    std::sort(vec_cv_direction_clusters.begin(),
              vec_cv_direction_clusters.end(),
              [&](PlaneNormalCluster a, PlaneNormalCluster b) {
                return a.count > b.count;
              });
 ...
 * mark others plane based on main_vertical_normals
 */
void StairDetection::filterPlanesWithKeyInfo(std::vector<Plane> &vector_plane,
                                             KeyInfo &keyInfo) {
  for (auto &i : vector_plane) {
...
void StairDetection::findConnectFragment(
    std::vector<Plane> &vector_plane,
    const std::vector<std::vector<Relation>> &confuse_matrix, KeyInfo &keyInfo,
    std::vector<int2> &fragment_vector) {
  for (size_t i = 0; i < vector_plane.size(); i++) {
    for (size_t j = i + 1; j < vector_plane.size(); j++) {
      if (vector_plane[i].ptype == Plane::pstair_component &&
        ...
        // the angle between the two plane center vector and
        // main_center_diff_vector
        float dot_product = keyInfo.main_center_diff_vector.normal.dot(dir);
        float angle = acos(dot_product);

        // TODO: stair_cv_angle_diff_th and stair_plane_max_distance
        if ((angle < stair_cv_angle_diff_th) &&
            (confuse_matrix[i][j].distance < stair_plane_max_distance) &&
 ...
 * find (or stitch) the longest from the connect plane fragments
 */
void StairDetection::findStairPlaneIndexList(
    std::vector<int2> &fragment_vector,
    std::vector<int> &stair_plane_index_list) {
  if (fragment_vector.size() > 1) {
    ...
    for (size_t i = 0; i < fragment_vector.size(); i++) {
      if (fragment_vector[i].from != last_from) {
        ...
        start_index_list.push_back(
            -1);  // -1 is a virtual point, for easy logic in findFragments
        start_index_list.push_back(
            fragment_vector[i].from);  // start from fragment_vector[i].from
        vector_index_list_i.push_back(start_index_list);

        findFragments(fragment_vector, i - 1, vector_index_list_i, 0);

        vector_index_list.insert(vector_index_list.end(),
                                 vector_index_list_i.begin(),
                                 vector_index_list_i.end());
      }
      last_from = fragment_vector[i].from;
    }

    for (auto &i : vector_index_list) i.erase(i.begin());
...
void StairDetection::findFragments(
    std::vector<int2> &fragment_vector, int index,
    std::vector<std::vector<int>> &vector_index_list, int vec_index) {
  // destination
  int start_from =
      vector_index_list[vec_index][vector_index_list[vec_index].size() - 2];
  int start_to =
      vector_index_list[vec_index][vector_index_list[vec_index].size() - 1];
  ...
  for (int i = index + 1; i < fragment_vector.size(); i++) {
    ...
    if (fragment_vector[i].from == start_to) {
      if (first) {
        ...
        if (i < fragment_vector.size() - 1)
          findFragments(fragment_vector, i, vector_index_list,
                        vector_index_list.size() - 1);
      }
    }
 ...
 * mark plane and TODO: refine the main vertical normal?
 */
void StairDetection::filterPlanesWithStairPlaneList(
    std::vector<Plane> &vector_plane,
    std::vector<int> &stair_plane_index_list) {
  for (size_t i = 0; i < vector_plane.size(); i++) {
    if (vector_plane[i].ptype == Plane::pstair_component) {
...
void StairDetection::computeStairSidePlane(std::vector<Plane> &vector_plane,
  ...
  if (!vec_right_intercepts.empty()) {
    std::vector<InterceptCount> vec_left_intercept_counts,
        vec_right_intercept_counts;
        ...
        if (fabs(vec_left_intercept - vec_left_intercept_count.intercept) <
          ...
          vec_left_intercept_count.intercept =
              (vec_left_intercept_count.intercept *
               ...
               vec_left_intercept) /
              (vec_left_intercept_count.count + 1);
          vec_left_intercept_count.count++;
        }
      }
        ...
        if (fabs(vec_right_intercept - vec_right_intercept_count.intercept) <
          ...
          vec_right_intercept_count.intercept =
              (vec_right_intercept_count.intercept *
               ...
               vec_right_intercept) /
              (vec_right_intercept_count.count + 1);
          vec_right_intercept_count.count++;
        }
      }
    ...
    sort(vec_left_intercept_counts.begin(), vec_left_intercept_counts.end(),
         [&](InterceptCount a, InterceptCount b) { return a.count > b.count; });

    sort(vec_right_intercept_counts.begin(), vec_right_intercept_counts.end(),
         [&](InterceptCount a, InterceptCount b) { return a.count > b.count; });

    if (vec_left_intercept_counts.size() > 1)
      if (vec_left_intercept_counts[0].count ==
          vec_left_intercept_counts[1].count)
        if (abs(vec_left_intercept_counts[0].intercept) <
            abs(vec_left_intercept_counts[1].intercept))
          swap(vec_left_intercept_counts[0], vec_left_intercept_counts[1]);

    if (vec_right_intercept_counts.size() > 1)
      if (vec_right_intercept_counts[0].count ==
          vec_right_intercept_counts[1].count)
        if (abs(vec_right_intercept_counts[0].intercept) <
            abs(vec_right_intercept_counts[1].intercept))
    ...
    keyInfo.side_plane_right.values[2] =
        keyInfo.horizontal_plane_direction.normal[2];
    keyInfo.side_plane_right.values[3] =
        vec_right_intercept_counts[0].intercept;
  }
}
 ...
 * modeling stair
 */
bool StairDetection::modelingStair(std::vector<Plane> &vector_plane,
                                   std::vector<int> &stair_plane_index_list,
  ...
  if (stair_plane_index_list.empty()) return false;
  ...
  for (size_t i = 0; i < stair_plane_index_list.size() - 1; i++) {
    ...
    if (find_ground && !i) {
      ...
      if (vector_plane[index].type == Plane::horizontal) {
        Eigen::Vector3f center_diff(
            vector_plane[gound_index].center.x - vector_plane[index].center.x,
        ...
        if (fabs(dot_product) < 2.5 * ground_height_th) {
          ...
          computeLineFrom2Planes(plane1_coefficients, plane2_coefficients,
                                 keyInfo, line1);

          p_concaveline->concave_line.plane_h = &vector_plane[gound_index];
          p_concaveline->concave_line.plane_v =
              &vector_plane[vector_plane.size() - 1];
    ...
    if ((vector_plane[index].type == Plane::horizontal) &&
      ...
      computeLineFrom2Planes(plane1_coefficients, plane2_coefficients, keyInfo,
                             line);
 ...
 * \param[out] point: max_point and min_point
 */
inline void StairDetection::findMinMaxProjPoint(
    const Plane &plane, const Eigen::Vector3f &ve, PointType &min_point,
    PointType &max_point, float &min_proj, float &max_proj) {
...
void StairDetection::computeLineFrom2Planes(
    ...
    const pcl::ModelCoefficients &coefficients_plane2, KeyInfo keyInfo,
    Line &line) {
  ...
  // compute cross point between line and plane perpendicular to this line with
  // in origin
  float t = -1 *
            (normal_line[0] * x0 + normal_line[1] * y0 + normal_line[2] * z0) /
            (normal_line[0] * normal_line[0] + normal_line[1] * normal_line[1] +
             normal_line[2] * normal_line[2]);
...
inline void StairDetection::crossPointOfLineAndPlane(
    ...
    const pcl::ModelCoefficients plane_coefficients, pcl::PointXYZ &pc) {
  float A, B, C, D;
  ...
  while (stair.readNext(pnext)) {
    if (pnext->pcurrent_type == step_node) {
      ...
      if (pstep->step.plane_v->counter.width == 0) {
        ...
        findMinMaxProjPoint(*pstep->step.plane_v, down_direction, min_point,
        ...
        // 左下侧交点
        crossPointOfLineAndPlane(xd, yd, zd, ndx, ndy, ndz,
                                 keyInfo.side_plane_left, point);
        pstep->step.plane_v->counter.points[0] = point;
        // 右下侧交点
        crossPointOfLineAndPlane(xd, yd, zd, ndx, ndy, ndz,
                                 keyInfo.side_plane_right, point);
        pstep->step.plane_v->counter.points[1] = point;
        // 右上侧交点
        crossPointOfLineAndPlane(xu, yu, zu, nux, nuy, nuz,
                                 keyInfo.side_plane_right, point);
      ...
      if (pstep->step.plane_h->counter.width == 0) {
        ...
        // 左前侧交点
        crossPointOfLineAndPlane(xf, yf, zf, nfx, nfy, nfz,
                                 keyInfo.side_plane_left, point);
        ...
        if (pnext->pnext->pnext == nullptr) {
          ...
          // 右后侧交点
          crossPointOfLineAndPlane(xb, yb, zb, nbx, nby, nbz,
                                   keyInfo.side_plane_right, point);
      ...
      if (pline->concave_line.plane_v->counter.width == 0) {
        ...
        if (pnext->pnext->pnext == nullptr) {
          ...
          PointType max_point, min_point;
          float max_prj, min_prj;
          findMinMaxProjPoint(*pline->concave_line.plane_v, down_direction,
                              min_point, max_point, min_prj, max_prj);
...
void StairDetection::setDown_sample_points_number(
    int down_sample_points_number) {
...
void StairDetection::setVertical_plane_normal_angle_th(
    float vertical_plane_normal_angle_th) {
  StairDetection::vertical_plane_normal_angle_th =
      vertical_plane_normal_angle_th;
}
...
void StairDetection::setVertical_plane_normal_est_angle_th(
    float vertical_plane_normal_est_angle_th) {
  StairDetection::vertical_plane_normal_est_angle_th =
      vertical_plane_normal_est_angle_th;
}
...
void StairDetection::setStair_plane_max_distance(
    float stair_plane_max_distance) {
...
void StairDetection::setMin_ground_plane_points_num(
    int min_ground_plane_points_num) {
