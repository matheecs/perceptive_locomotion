...
enum PartType { concaveline_node, step_node, start_or_end_node };

class Node {
 public:
  Node()
      : pcurrent_type(PartType::start_or_end_node),
        pnext_type(PartType::start_or_end_node),
        pnext(nullptr) {}
...
class Stair {
 public:
  ...
  std::shared_ptr<Node> getHead() { return phead; }

  std::shared_ptr<Node> getCurPoint() { return point_current; }
