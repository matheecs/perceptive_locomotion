#ifndef STAIRMODELING_HPP_
#define STAIRMODELING_HPP_
...
class StairModeling {
 public:
  ...
  void planeOdomCallback(const plane_msg::VecPlane &vec_plane,
    ...
    if (!vec_plane.vecPlane.empty()) {
      ...
      convertCloud(vec_plane, vsp_plane, camera_position);
      ...
      if (has_stair) {
        ...
        stair_record << vec_plane.vecPlane[0].cloud.header.stamp << ",";
        stair_record << stair_detector.getEstimatedParamStringRcd(stair);

        send2robot(stair, vec_plane.vecPlane[0].cloud.header.stamp);

        publish_rviz_stair(vsp_plane, vec_plane.vecPlane[0].cloud.header.stamp);
      } else {
        detial_stair_model = "No stair";
        est_stair_param = "No stair";
      }

      time_record << vec_plane.vecPlane[0].cloud.header.stamp << ",";
