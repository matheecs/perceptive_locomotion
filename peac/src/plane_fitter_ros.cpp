...
class peac_ros {
 public:
  peac_ros(const ros::NodeHandle &private_nh = ros::NodeHandle("~"),
      ...
      : queueSize(5),
        topicPointCloud2(topicPointCloud2),
        show_viewer(true),
        running(true),
        topicAdImage(topicAdImage),
        ...
        distance_min(50),
        distance_max(500),
        server(new dynamic_reconfigure::Server<peac::peac_paramConfig>(
            private_nh)) {}

  ~peac_ros() {
    running = false;
    if (show_viewer) viz_thread.join();
  }

  void start() {
    ROS_INFO("############# peac_ros start #############");

    sub = private_nh.subscribe(topicPointCloud2, queueSize, &peac_ros::callback,
                               this);
    ...
    pub_vec_planes =
        private_nh.advertise<plane_msg::VecPlane>(topicVecPlane, queueSize);
  ...
  void cfg_callback(peac::peac_paramConfig &config, uint32_t level) {
    ...
    pf.params.similarityTh_merge =
        std::cos(MACRO_DEG2RAD(config.similarityTh_merge));
    pf.params.similarityTh_refine =
        std::cos(MACRO_DEG2RAD(config.similarityTh_refine));
  ...
  void callback(const sensor_msgs::PointCloud2::ConstPtr cloud_ros) {
    ...
    cv::Mat seg(cloud_in_pcl_xyzrgba.height, cloud_in_pcl_xyzrgba.width,
                CV_8UC3);
  ...
  void MatImg2RosImage(const cv::Mat &cvmat, sensor_msgs::Image &rosimg,
                       std::string encoding) {
  ...
  void Ixyz2RosImage(const ImageXYZRGBA &Ixyzrgba, sensor_msgs::Image &rosimg,
                     double scale) {
  ...
  void GenerateVecPlanes(const std::vector<std::vector<int>> &pMembership,
    ...
    for (size_t i = 0; i < plane_number; i++) {
      ...
      // we do not publish the statistics of plane here, only publish the plane
      // points the statistics will computed in the process node
    }
  }
    ...
    for (size_t i = 0; i < height; i++) {
      for (size_t j = 0; j < width; j++) {
        ...
        if (distance2 < distance_min * distance_min ||
            distance2 > distance_max * distance_max) {
          cloud.points[i * width + j].x =
      ...
      if (!vsp_plane_viz.vecPlane.empty()) {
        ...
        for (unsigned int i = 0; i < vsp_plane_viz.vecPlane.size(); i++) {
          ...
          if (pointCloud.points.size()) {
            // add cloud
            ss << "c";
            viewer->addPointCloud<pcl::PointXYZRGBA>(pointCloud.makeShared(),
                                                     single_color, ss.str());
...
class peac_ros_nodelet : public nodelet::Nodelet {
 ...
 public:
  ...
  ~peac_ros_nodelet() { delete ppeac_ros; }
...
PLUGINLIB_DECLARE_CLASS(peac, peac_ros_nodelet, plane_fitter::peac_ros_nodelet,
                        nodelet::Nodelet);
}  // namespace plane_fitter
...
int main(int argc, char **argv) {
  ...
  //    plane_fitter::peac_ros
  //    peacRos(private_nh,"/peac_cloud_rotation/cloud_xyzrgba");
